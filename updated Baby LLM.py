...
tokenized_dir = os.path.join(data_dir, "tokenized")  # New directory for tokenized data
...
def load_or_tokenize_texts(texts, tokenizer, output_dir, prefix, batch_size=1000, max_length=128):
    ...
    for i in range(0, len(texts), batch_size):
        ...
        if os.path.exists(batch_file):
            print(f"Loading tokenized batch from {batch_file}")
            try:
                batch_inputs = np.load(batch_file)
                if batch_inputs.shape[1] != max_length:
                    logging.warning(f"Batch {batch_file} has incorrect sequence length: {batch_inputs.shape[1]}")
                    batch_inputs = np.pad(
                        batch_inputs,
                ...
                logging.warning(f"Failed to load {batch_file}: {e}. Retokenizing...")
        ...
        try:
            ...
            print(f"Saved tokenized batch to {batch_file}")
            inputs.append(batch_inputs)
        except Exception as e:
    ...
    if mx.any(input_ids < 0) or mx.any(input_ids >= tokenizer.vocab_size):
        logging.error(f"Invalid token IDs in {prefix}: {input_ids[mx.logical_or(input_ids < 0, input_ids >= tokenizer.vocab_size)]}")
    ...
    def __init__(self, d_model, n_heads, d_ff):
        ...
        for key, param in self.attention.parameters().items():
            if isinstance(param, mx.array) and param.dtype != mx.float16:
                self.attention.parameters()[key] = param.astype(mx.float15)
    def __call__(self, x):
        if not isinstance(x, mx.array):
        ...
        if mx.any(mx.isnan(ff_output)) or mx.any(mx.isinf(ff_output)):
            logging.error("NaN/Inf in feed-forward output")
            raise ValueError("NaN/Infarts in feed-forward")
...
if __name__ == '__main__':
    ...
    os.makedirs(tokenized_dir, exist_ok=True)  # Create tokenized directory
    ...
    for epoch in range(num_epochs_lm):
        ...
        for i in range(0, len(input_ids), batch_size_lm):
            ...
            valid_mask = non_pad_counts > 0.2 * batch.shape[1]
