...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, n_heads, d_ff):
        ...
        self.norm1 = nn.LayerNorm(d_model, eps=1e-5)  # Increase epsilon
    ...
    def __call__(self, x, mask=None, padding_mask=None):
        ...
        if padding_mask is not None:
            ...
            if padding_mask.shape[1] != x.shape[1]:
                logging.error(f"Padding mask shape {padding_mask.shape} does not match input sequence length {x.shape[1]}")
...
class BabyLLM(nn.Module):
    ...
    def __call__(self, x):
        ...
        # Create padding mask
        padding_mask = (x != tokenizer.pad_token_id).astype(mx.bool_)
        padding_mask = padding_mask[:, None, :]  # Shape: (batch_size, 1, seq_len)
        non_pad_counts = mx.sum(padding_mask, axis=2).flatten()
        ...
        for i, layer in enumerate(self.layers):
            try:
                ...
                x = mx.clip(x, -1e2, 1e2)  # Add clipping after each layer
...
if __name__ == '__main__':
    ...
    try:
        ...
        if tokenizer.pad_token_id == tokenizer.bos_token_id:
            logging.warning("pad_token_id is set to bos_token_id, changing to eos_token_id")
            tokenizer.pad_token = tokenizer.eos_token
            tokenizer.pad_token_id = tokenizer.eos_token_id

        # Verify tokenizer special tokens
        logging.info(f"Tokenizer special tokens: bos={tokenizer.bos_token_id}, eos={tokenizer.eos_token_id}, pad={tokenizer.pad_token_id}, sep={tokenizer.sep_token_id}")
        if tokenizer.pad_token_id is None or tokenizer.pad_token_id == tokenizer.bos_token_id:
            logging.error("Invalid pad token configuration")
            raise ValueError("Invalid pad token configuration")
        ...
        for i, t in enumerate(texts):
            ...
            if current_size + size <= max_size and len(t.strip()) >= 10:
                try:
                    ...
                    token_ids = tokenizer(t, truncation=True, add_special_tokens=True)["input_ids"]
                    if 5 <= len(token_ids) <= 1280:  # Cap at 10x max_length
...
# ... QA pairs, validation prompts, and pretraining loop follow ...
        ...
        def is_valid_qa_pair(question, answer):
            ...
            try:
                ...
                if tokens.shape[1] > 128 or non_pad_count < 6:  # Require at least 6 non-padding tokens
        ...
        num_epochs_lm = 2 #Will skipping help?
        ...
        logging.info("Initialized generative_model (BabyLLM)")

        # Load pretrained weights
        weights_path = os.path.join(model_dir, f"baby_llm_pretrain_epoch_{num_epochs_lm-1}.npz")
        if os.path.exists(weights_path):
            try:
                ...
                logging.info(f"Loaded pretrained weights from {weights_path}")
            except Exception as e:
                logging.warning(f"Failed to load pretrained weights: {str(e)}. Reinitializing embeddings only.")
                std = 0.005  # Smaller std for stability
                ...
                generative_model.pos_embedding.weight = mx.random.normal(
                    generative_model.pos_embedding.weight.shape, loc=0.0, scale=std, dtype=mx.float16
                )
        else:
            logging.warning(f"Pretrained weights not found at {weights_path}. Reinitializing embeddings only.")
            std = 0.005  # Smaller std for stability
            generative_model.embedding.weight = mx.random.normal(
                generative_model.embedding.weight.shape, loc=0.0, scale=std, dtype=mx.float16
            )
        ...
        # Validate parameters
        for name, param in generative_model.parameters().items():
            if isinstance(param, mx.array):
                if mx.any(mx.isnan(param)) or mx.any(mx.isinf(param)):
                    logging.error(f"NaN/Inf in parameter {name} after initialization")
                    np.save(os.path.join(model_dir, f"invalid_param_{name}_{time.time()}.npy"), np.array(param))
                    raise ValueError(f"Invalid parameter {name}")
                logging.debug(f"Parameter {name}: shape={param.shape}, min={mx.min(param).item()}, max={mx.max(param).item()}")
        mx.eval(generative_model.parameters())

        # Test forward pass with val_input_ids
        logging.info("Testing BabyLLM forward pass with val_input_ids")
        test_input = val_input_ids[:, :-1]
        decoded_test = [tokenizer.decode(to_numpy_for_decode(test_input[i])) for i in range(test_input.shape[0])]
        logging.info(f"Test input decoded: {decoded_test}")
        test_output = generative_model(test_input)
        if test_output is None:
            logging.error("Test forward pass returned None")
            np.save(os.path.join(model_dir, f"test_failed_input_{time.time()}.npy"), np.array(test_input))
            raise ValueError("Test forward pass failed")
        else:
            logging.info(f"Test forward pass output: shape={test_output.shape}, min={mx.min(test_output).item()}, max={mx.max(test_output).item()}")
        ...
        for epoch in range(start_epoch_lm, num_epochs_lm):
            ...
            for i in range(0, len(input_ids), batch_size_lm):
                ...
                if batch_indices.shape[0] < batch_size_lm:
                    ...
                    continue
                if mx.any(batch_indices < 0) or mx.any(batch_indices >= len(input_ids)):
                    logging.warning(f"Invalid batch indices in pretraining batch {batch_idx}: {batch_indices.tolist()}")
                    continue
                ...
                if not mx.any(valid_mask):
                    logging.warning(f"Pretraining batch {batch_idx} has excessive padding. Skipping.")
                    continue

                # Use NumPy for boolean indexing
                valid_mask_np = np.array(valid_mask)
                valid_indices_np = np.where(valid_mask_np)[0]
                if valid_indices_np.size < 2:
                    logging.warning(f"Pretraining batch {batch_idx} has too few valid sequences ({valid_indices_np.size}). Skipping.")
                    continue
                valid_indices = mx.array(valid_indices_np, dtype=mx.int32)
        ...
        # Validate model weights post-pretraining
        for name, param in generative_model.parameters().items():
            if isinstance(param, mx.array):
                if mx.any(mx.isnan(param)) or mx.any(mx.isinf(param)):
                    logging.error(f"NaN/Inf in parameter {name} after pretraining")
                    np.save(os.path.join(model_dir, f"invalid_weights_{name}_{time.time()}.npy"), np.array(param))
                    raise ValueError(f"Invalid weights in {name}")
                logging.debug(f"Parameter {name}: shape={param.shape}, min={mx.min(param).item()}, max={mx.max(param).item()}")
        ...
        optimizer_gen = optim.AdamW(learning_rate=CosineWarmup(5e-7, 1000, total_steps_qa), weight_decay=0.05)
        patience = 10
        best_val_em = 0.0
        patience_counter = 0
        val_losses_gen = []
        ...
        if not mx.any(valid_mask):
            logging.warning("No valid sequences in val_corpus_ids. Using all sequences.")
            valid_indices = mx.arange(val_corpus_ids.shape[0], dtype=mx.int32)
        else:
        ...
        # Reinitialize and validate embedding weights
        std = 0.02
        generative_model.embedding.weight = mx.random.normal(
            generative_model.embedding.weight.shape, loc=0.0, scale=std, dtype=mx.float16
        )
        ...
        if mx.any(mx.isnan(generative_model.embedding.weight)) or mx.any(mx.isinf(generative_model.embedding.weight)):
            logging.error("NaN/Inf in reinitialized embedding weights")
            raise ValueError("Invalid embedding weights after reinitialization")
        if mx.any(mx.isnan(generative_model.pos_embedding.weight)) or mx.any(mx.isinf(generative_model.pos_embedding.weight)):
            logging.error("NaN/Inf in reinitialized positional embedding weights")
            raise ValueError("Invalid positional embedding weights after reinitialization")
        logging.info(f"Reinitialized embedding weights: shape={generative_model.embedding.weight.shape}, "
                    f"min={mx.min(generative_model.embedding.weight).item()}, "
                    f"max={mx.max(generative_model.embedding.weight).item()}")
        logging.info(f"Reinitialized pos_embedding weights: shape={generative_model.pos_embedding.weight.shape}, "
                    f"min={mx.min(generative_model.pos_embedding.weight).item()}, "
                    f"max={mx.max(generative_model.pos_embedding.weight).item()}")
        np.save(os.path.join(model_dir, f"embedding_weights_{time.time()}.npy"), np.array(generative_model.embedding.weight))
        np.save(os.path.join(model_dir, f"pos_embedding_weights_{time.time()}.npy"), np.array(generative_model.pos_embedding.weight))
        mx.eval(generative_model.parameters())
        # Validate val_input_ids before fine-tuning
        logging.info(f"Validating val_input_ids: shape={val_input_ids.shape}, "
                    f"min={mx.min(val_input_ids).item()}, max={mx.max(val_input_ids).item()}")
        vocab_size_with_special = generative_model.embedding.weight.shape[0]
        sliced_input = val_input_ids[:, :-1]
        logging.info(f"Validating sliced val_input_ids: shape={sliced_input.shape}, "
                    f"min={mx.min(sliced_input).item()}, max={mx.max(sliced_input).item()}")
        if mx.any(sliced_input < 0) or mx.any(sliced_input >= vocab_size_with_special):
            logging.error(f"Invalid tokens in sliced val_input_ids: min={mx.min(sliced_input).item()}, "
                         f"max={mx.max(sliced_input).item()}, vocab_size={vocab_size_with_special}")
            np.save(os.path.join(model_dir, f"invalid_val_input_ids_{time.time()}.npy"), np.array(sliced_input))
            raise ValueError("Invalid tokens in sliced val_input_ids")
        if sliced_input.shape[1] > generative_model.pos_embedding.weight.shape[0]:
            logging.error(f"Sliced val_input_ids sequence length {sliced_input.shape[1]} exceeds max_len {generative_model.pos_embedding.weight.shape[0]}")
            raise ValueError("Sequence length exceeds max_len")
        
        logging.info("Validating model parameters before fine-tuning")
        for name, param in generative_model.parameters().items():
            if isinstance(param, mx.array):
                if mx.any(mx.isnan(param)) or mx.any(mx.isinf(param)):
                    logging.error(f"NaN/Inf in parameter {name} before fine-tuning")
                    np.save(os.path.join(model_dir, f"invalid_param_{name}_{time.time()}.npy"), np.array(param))
                    raise ValueError(f"Invalid parameter {name}")
                logging.debug(f"Parameter {name}: shape={param.shape}, min={mx.min(param).item()}, max={mx.max(param).item()}")

        # Reinitialize all model parameters
        ##logging.info("Reinitializing all model parameters before fine-tuning")
        ##std = 0.02
        ##for name, param in generative_model.parameters().items():
        ##    if isinstance(param, mx.array):
         ##       generative_model.parameters()[name] = mx.random.normal(
         ##           shape=param.shape, loc=0.0, scale=std, dtype=mx.float16
         ##       )
        ##        logging.debug(f"Reinitialized {name}: shape={param.shape}, min={mx.min(param).item()}, max={mx.max(param).item()}")
        mx.eval(generative_model.parameters())
        ...
        for epoch in range(num_epochs_qa):
            ...
            for i in range(0, len(indices), batch_size_qa):
                ...
                if mx.any(batch < 0) or mx.any(batch >= vocab_size_with_special):
                    ...
                    continue
                non_pad_counts = mx.sum(batch != tokenizer.pad_token_id, axis=1)
                logging.info(f"QA batch {batch_idx} non_pad_counts: {non_pad_counts.tolist()}")
                valid_mask = non_pad_counts > 0.1 * batch.shape[1]  # Lowered threshold
                logging.info(f"QA batch {batch_idx} valid_mask shape: {valid_mask.shape}, values: {valid_mask.tolist()}, dtype: {valid_mask.dtype}")
                ...
                if not mx.any(valid_mask):
                    logging.warning(f"QA batch {batch_idx} has excessive padding. Skipping filtering.")
                    valid_indices = mx.arange(batch.shape[0], dtype=mx.int32)
                else:
                    # Use NumPy for boolean indexing
                    valid_mask_np = np.array(valid_mask)
                    valid_indices_np = np.where(valid_mask_np)[0]
                    if valid_indices_np.size == 0 or valid_indices_np.size < 2:
                        logging.warning(f"QA batch {batch_idx} has no valid sequences or too few sequences ({valid_indices_np.size}). Skipping.")
                        continue
                    valid_indices = mx.array(valid_indices_np, dtype=mx.int32)
            ...
            logging.info(f"Before loss_fn_qa, val_input_ids shape: {val_input_ids.shape}, "
                        f"min: {mx.min(val_input_ids).item()}, max: {mx.max(val_input_ids).item()}")
            for i in range(val_input_ids.shape[0]):
                decoded = tokenizer.decode(to_numpy_for_decode(val_input_ids[i]))
                logging.info(f"val_input_ids[{i}]: {decoded}")
            if mx.any(val_input_ids < 0) or mx.any(val_input_ids >= vocab_size_with_special):
                logging.error(f"Invalid tokens in val_input_ids: {val_input_ids.tolist()}")
                np.save(os.path.join(model_dir, f"invalid_val_input_ids_loss_fn_qa_{time.time()}.npy"), np.array(val_input_ids))
                raise ValueError("Invalid tokens in val_input_ids before loss_fn_qa")
            if val_input_ids.shape[1] > generative_model.pos_embedding.weight.shape[0]:
                logging.error(f"val_input_ids sequence length {val_input_ids.shape[1]} exceeds max_len {generative_model.pos_embedding.weight.shape[0]}")
                raise ValueError("Sequence length exceeds max_len in loss_fn_qa")
